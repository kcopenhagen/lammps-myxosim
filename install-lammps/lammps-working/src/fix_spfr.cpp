...
FixSPFR::FixSPFR(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  ...
  rev_sig = utils::numeric(FLERR, arg[6], false, lmp);
...
void FixSPFR::pre_force(int /*vflag*/)
{
  ...
  for (i = 0; i < nlocal; i++) {
    if (ttr[i] <= 0)
      ttr[i] = rng->gaussian(rev_mu/dt, rev_sig/100.0*rev_mu/dt);
  }
}

/* ---------------------------------------------------------------------- */

void FixSPFR::post_force(int /*vflag*/)
{
  double **f = atom->f;
  double **x = atom->x;
