...
enum{ID,MOL,PROC,PROCP1,TYPE,ELEMENT,MASS,
     ...
     IX,IY,IZ,
     VX,VY,VZ,FX,FY,FZ,
     Q,MUX,MUY,MUZ,MU,AF,NUM_BOND,RADIUS,DIAMETER,
  ...
  if (nthresh) {
    ...
    for (int ithresh = 0; ithresh < nthresh; ithresh++) {
      ...
      if (thresh_array[ithresh] == ID) {
        ...
        ptr = &atom->mu[0][3];
        nstride = 4;
      } else if (thresh_array[ithresh] == AF) {
        ptr = &atom->af[0];
        nstride = 1;
...
int DumpCustom::parse_fields(int narg, char **arg)
{
  ...
  for (int iarg = 0; iarg < narg; iarg++) {
    if (strcmp(arg[iarg],"id") == 0) {
      ...
      vtype[iarg] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"af") == 0) {
      pack_choice[iarg] = &DumpCustom::pack_af;
      vtype[iarg] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"num_bond") == 0) {
      pack_choice[iarg] = &DumpCustom::pack_num_bond;
      vtype[iarg] = Dump::INT;
...
int DumpCustom::modify_param(int narg, char **arg)
{
  ...
  if (strcmp(arg[0],"thresh") == 0) {
    ...
    else if (strcmp(arg[1],"af") == 0) thresh_array[nthresh] = AF;
    else if (strcmp(arg[1],"num_bond") ==0) thresh_array[nthresh] = NUM_BOND;
...
/* ---------------------------------------------------------------------- */
void DumpCustom::pack_num_bond(int n)
{
  int *num_bond = atom->num_bond;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = num_bond[clist[i]];
    n += size_one;
  }
}
